...
echo "$($PSQL "SELECT DISTINCT(name) FROM ((SELECT winner_id FROM games WHERE (round = 'Eighth-Final' AND year = 2014)) UNION (SELECT opponent_id FROM games WHERE (round = 'Eighth-Final' AND year = 2014))) AS b INNER JOIN teams ON b.winner_id = teams.team_id ORDER BY name")"
